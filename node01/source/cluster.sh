...
fun_init_cluster() {
    ...
    for i in {30..0}; do
        flag=$(docker exec -it "${CONTAINER_RABBITMQ}" rabbitmqctl ping | grep succeeded | wc -l || true)
        if [[ ${flag} > 0 ]]; then
            echo 'ok'
            break
        fi
    ...
    docker exec -it "${CONTAINER_RABBITMQ}" rabbitmqctl stop_app
    ...
    info "rabbitmqctl set_cluster_name"
    docker exec -it "${CONTAINER_RABBITMQ}" rabbitmqctl set_cluster_name "${CLUSTER_NAME_RABBITMQ}"
    info "rabbitmqctl set_policy"
    docker exec -it "${CONTAINER_RABBITMQ}" rabbitmqctl set_policy -p / ha-all "^" '{"ha-mode":"all","ha-sync-mode":"automatic","ha-promote-on-shutdown":"always","ha-promote-on-failure":"always"}'
